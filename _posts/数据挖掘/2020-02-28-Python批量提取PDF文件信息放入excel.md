---
layout : post
title : Python批量提取PDF文件信息放入excel
date : '2020-02-28 15:05:00 +0800'
categories: Python
---
...
程序运行逻辑：
   ...
   -->遍历指定的PDF文件夹，将其名称与Excel中得到的进行匹配，若匹配成功，则保存该文件或文件夹的路径到folderList.txt中

   -->遍历上述文件夹内的按照指定规则获得的所有PDF文件并保存到pdfList.txt中

   -->解析上面获得的PDF文件并保存到PDF2Txt文件夹中
   
   -->按照指定的规则抽取d步骤获得的TXT文件中的内容

   -->把e步骤获得的内容写入到对应的Excel位置
...
比如pyinstaller.exe 放在了C:/Users/github/Python 路径下，同时里面还有一个TEST.py 的文件我想要打包成exe 程序，那么我的运行命令就是：
...
````YMAL 
#%%cell1
#蒋手工配置好的要求信息读入，主要包括各种文件提取规则，提取内容和文件的读写路径
#加载配置文件
...
>源地址：[提取PDF内容保存到Excel--Python实现](https://blog.csdn.net/zyc121561/article/details/77207038 "提取PDF内容保存到Excel--Python实现")
